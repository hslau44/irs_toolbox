...
from train import evaluation,cmtx_table,make_directory,save_checkpoint
...
def record_log(mode,epochs,record,cmtx,cls):
    if mode == 'pretrain':
        path = make_directory(exp_name+'_pretrain',epoch=epochs,filepath='./record/')
        pd.DataFrame(record['train'],columns=['train_loss']).to_csv(path+'_loss.csv')
    elif mode == 'finetuning':
        path = make_directory(exp_name+'_finetuning',epoch=epochs,filepath='./record/')
        pd.DataFrame(record['train'],columns=['train_loss']).to_csv(path+'_loss.csv')
...
def save(mode,model,optimizer,epochs):
    if mode == 'pretrain':
        path = make_directory(exp_name+'_pretrain',epoch=epochs)
        save_checkpoint(model, optimizer, epochs, path)
    elif mode == 'finetuning':
        path = make_directory(exp_name+'_finetuning',epoch=epochs)
        save_checkpoint(model, optimizer, epochs, path)
    return
...
def main():
    ...
    # Fine-tuning
    finetune_model = create_finetune_model(model.encoder)
    criterion = nn.CrossEntropyLoss()
    optimizer = create_optimizer('finetuning',pretrain_model)
